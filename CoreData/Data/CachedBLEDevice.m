...
//            }
    ...
    if (changed == YES)
    {
         [[MTKBleProximityService getInstance] updatePxpSetting:mDeviceIdentifier alertEnabler:_mAlertEnabled range:2 rangeType:_mRangeType alertDistance:_mRangeValue disconnectAlertEnabler:_mDisconnectEnabled];
...
-(void)updateDeviceConnectionState:(CBPeripheral*)peripheral connectionState:(int)state;
{
    ...
    {
        NSLog(@"[CachedBLEDevice] [updateDeviceConnectionState] peripheral identifier not match  %@ , %@",[[peripheral identifier] UUIDString],mDeviceIdentifier);
        return;
    }
